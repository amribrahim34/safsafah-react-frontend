...
export default function MapPickerLite({ lang, brand, onPick }: MapPickerProps) {
  ...
  useEffect(() => {
    ...
    ensureLeaflet().then(() => {
      ...
      const setPoint = async (latlng: { lat: number; lng: number }) => {
      ...
      mapInst.current.on("click", (e: any) => setPoint(e.latlng));

      // geolocate helper
      const tryLocate = () => {
        mapInst.current.locate({ setView: true, maxZoom: 15 });
      };
      mapInst.current.on("locationfound", (e: any) => setPoint(e.latlng));
      ...
      btn.onAdd = () => {
        ...
        a.innerHTML = "◎";
        a.style.padding = "8px";
        a.onclick = (ev: Event) => { ev.preventDefault(); tryLocate(); };
        return div;
      };

...
export default function WalletPaymentPage() {
  ...
  const langFromState = state?.lang || "ar";
  const [lang, setLang] = useState(langFromState);
  const T = useMemo(() => COPY[lang], [lang]);

  const walletNumber = state?.walletNumber || "0100 000 0000";
  const orderRef = state?.orderRef || `SFS-${Date.now()}`;
  ...
  const submitProof = (e) => {
    ...
    nav("/"); // or to an order status page
  };
